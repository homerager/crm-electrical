export default defineComponent({
  name: 'EquipmentPrintPage',
  setup() {
    definePageMeta({ middleware: ['auth'] })
    useHead({ title: 'Друк QR-наклейок' })

    const { data, pending } = useFetch('/api/equipment')
    ...
    const filtered = computed(() => {
      if (!search.value) return equipment.value
      const s = search.value.toLowerCase()
      return equipment.value.filter((e: any) =>
    ...
    const selectedItems = computed(() =>
      equipment.value.filter((e: any) => selected.value.includes(e.id)),
    )
    ...
    function toggleAll() {
      if (selected.value.length === filtered.value.length) {
        ...
        selected.value = filtered.value.map((e: any) => e.id)
      }
    }
    ...
    return () => (
      <div>
        <div class="page-toolbar d-print-none">
          ...
          <div class="text-h5 font-weight-bold ml-2">Друк QR-наклейок</div>
          <v-spacer />
                  ...
                  default: ({ item }: any) => (
                    ...
                    >
                      {{
                        ...
                        subtitle: () => (
                          <span class="text-caption">{[item.model, item.serialNumber].filter(Boolean).join(' • ')}</span>
                        ),
                      }}
                    </v-list-item>
                  ),
            ...
            {selectedItems.value.map((item: any) => (
              <div key={item.id} class="qr-label">
                ...
                <div class="qr-label-info">
                  ...
                  {item.serialNumber && <div class="text-caption text-truncate">{item.serialNumber}</div>}
                </div>
              </div>
            ))}
          </div>
