  ...
  setup() {
    ...
    function openCreate() {
      ...
      Object.assign(form, {
        ...
        notes: '', taxCode: '', iban: '', bankName: '', bankMfo: '',
      })
      dialog.value = true
    }
    ...
    function clientHasPayment(item: any) {
      return !!(item?.taxCode || item?.iban || item?.bankName || item?.bankMfo)
    }
    ...
    return () => (
      <div>
        ...
        <v-card>
          <v-data-table headers={headers} items={clients.value} loading={pending.value} hover>
            {{
              'item.phone': ({ item }: any) => (
                item.phone
        ...
        <v-dialog v-model={dialog.value} max-width={640} scrollable>
          <v-card>
            <v-card-title>{editItem.value ? 'Редагувати клієнта' : 'Новий клієнт'}</v-card-title>
            <v-card-text>
              {error.value && <v-alert type="error" variant="tonal" class="mb-3">{error.value}</v-alert>}
              <v-text-field v-model={form.name} label="Назва / ПІБ *" class="mb-3" />
        ...
        <v-dialog v-model={deleteDialog.value} max-width={400}>
          <v-card>
            <v-card-title>Видалити клієнта?</v-card-title>
            <v-card-text>Клієнт "{deleteItem.value?.name}" буде видалено.</v-card-text>
            <v-card-actions class="pa-4 pt-0">
              <v-spacer />
